...
def attack_clean(state,model,carryover_budget_squared, clean_prev_obs_tens = None, dirty_prev_obs_tens = None):
	...
	if (clean_prev_obs_tens is not None):
		...
		target_logits =  model.q_net(state.repeat(1,4,1,1))[0]
	...
	for target in targets:
		...
		for i in range(step_count):
			...
			if (clean_prev_obs_tens is not None):
				...
				out = model.q_net(state.repeat(1,4,1,1))
...
if __name__ == '__main__':
	...
	eval_env =  TimeLimit(AtariPreprocessing(gym.make("FreewayHardNoFrameskip-v0")) ,250)
	...
	for i_episode in range(args.num_evals):
		...
		while True: 
			...
			action, _ = model.predict(torch.cat(frame_hist[-4:],dim=0),deterministic=True)
			#print(action)
